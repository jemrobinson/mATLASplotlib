...
class Histogram1D(BasePlottable):
    ...
    def draw_on_plot(self, axes, **kwargs):
        ...
        if kwargs['linestyle'] is 'dashed':
            kwargs['dashes'] = (3 * width, 1 * width)
        if kwargs['linestyle'] is 'dotted':
            kwargs['dashes'] = (1 * width, 1 * width)
        if kwargs['linestyle'] is 'dashdot':
            kwargs['dashes'] = (2 * width, 1 * width, 1 * width, 1 * width)
