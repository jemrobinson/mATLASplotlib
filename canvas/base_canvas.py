...
class BaseCanvas(object):
    ...
    def add_plottable(self, plottable, axes, **kwargs):
        ...
        if legend_text is '@':
            legend_text = kwargs.get('label', '')
        if 'stack' in kwargs.get('style', ''):
            legend_text = 'stack:' + legend_text
    ...
    def draw_ATLAS_text(self, x, y, axes, anchor_to='lower left', plot_type=None, coordinates='figure'):
        ...
        if plot_type is None:
            ...
            if ha == 'left':  # draw ATLAS first and then align other text to it
                ...
                self.plots[axes].text(bounding_box.min[0], y - 0.001, 'ATLAS', style='italic', fontsize=17, fontweight='bold', ha=ha, va=va, transform=transform)
            else:
    ...
    def translate_coordinates(self, coordinates, axes):
        if coordinates == 'axes':
            return self.plots[axes].transAxes
